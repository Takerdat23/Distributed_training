...
class Trainer:
    ...
    def prepare_dataloader(self, train_dataset, eval_dataset):
        ...
        if (self.is_ddp_training == True): 
            sampler= DistributedSampler(train_dataset)
        collate_fn = DataCollatorForSeq2Seq(tokenizer= self.tokenizer,padding = True ,  return_tensors="pt")
        data_trainloader = DataLoader(train_dataset, batch_size= self.batch_size, sampler=sampler, collate_fn=collate_fn ) ### YOUR CODE HERE ###
        ...
        collate_fn= DataCollatorForSeq2Seq(tokenizer=self.tokenizer, sampler = sampler , padding=True, return_tensors="pt")
        data_testloader = DataLoader(eval_dataset , batch_size=self.batch_size , sampler =sampler, collate_fn=collate_fn ) ### YOUR CODE HERE ###
...
def load_pretrained_model(local_rank, model_path: str = ""):
    ...
    model =AutoModelForCausalLM(model_path).to(torch.float16) ### YOUR CODE HERE ###
